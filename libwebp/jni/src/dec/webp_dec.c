...
WEBP_EXTERN uint8_t* WebPCustomDecode(const uint8_t* data, size_t data_size, int* width, int* height, int down_scaling, int mode) {
  int ret = WebPGetInfo(data, data_size, width, height);
  if (ret == 0) {
    return 0;
  }
  if (WebPInitDecoderConfig(&customConfig) == 0) {
    return 0;
  }
  customConfig.options.no_fancy_upsampling = 1;
  customConfig.options.use_threads = 1;
  if (down_scaling == 1) {
    *width/=2;
    *height/=2;
    customConfig.options.use_scaling = 1;
    customConfig.options.scaled_width = *width;
    customConfig.options.scaled_height = *height;
  }
  if (mode == 1) {
    customConfig.output.colorspace = MODE_RGBA;
  } else {
    customConfig.output.colorspace = MODE_RGB;
  }
  if (WebPDecode(data, data_size, &customConfig) == VP8_STATUS_OK) {
    return customConfig.output.u.RGBA.rgba;
  } else {
    return 0;
  }
}

WEBP_EXTERN void WebPCustomFree() {
  WebPFreeDecBuffer(&customConfig.output);
}